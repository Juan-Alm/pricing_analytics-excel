...
in
    ...
    #"Merged Queries2" = Table.NestedJoin(#"Added Custom8", {"product_id"}, dim_products, {"product_id"}, "dim_products", JoinKind.LeftOuter),
    #"Expanded dim_products1" = Table.ExpandTableColumn(#"Merged Queries2", "dim_products", {"product_name", "category", "subcategory", "brand", "base_price", "cost"}, {"product_name", "category", "subcategory", "brand", "base_price", "cost.1"}),
    #"Merged Queries3" = Table.NestedJoin(#"Expanded dim_products1", {"customer_id"}, dim_customers, {"customer_id"}, "dim_customers", JoinKind.LeftOuter),
    #"Expanded dim_customers1" = Table.ExpandTableColumn(#"Merged Queries3", "dim_customers", {"customer_name", "segment", "industry", "region_id"}, {"customer_name", "segment", "industry", "region_id.1"}),
    #"Merged Queries4" = Table.NestedJoin(#"Expanded dim_customers1", {"region_id"}, dim_regions, {"region_id"}, "dim_regions", JoinKind.LeftOuter),
    #"Expanded dim_regions" = Table.ExpandTableColumn(#"Merged Queries4", "dim_regions", {"region", "city", "market_type"}, {"region", "city", "market_type"}),
    #"Reordered Columns1" = Table.ReorderColumns(#"Expanded dim_regions",{"transaction_id", "product_id", "customer_id", "region_id", "cost", "final_price", "list_price", "revenue", "profit", "profit_flag", "discount_flag ", "discount_pct", "units_sold_flag", "units_sold", "date", "row_quality", "product_name", "category", "subcategory", "brand", "base_price", "customer_name", "segment", "industry", "region", "city", "market_type", "cost.1", "region_id.1"}),
    #"Removed Columns6" = Table.RemoveColumns(#"Reordered Columns1",{"cost.1", "region_id.1"}),
    #"Added Custom9" = Table.AddColumn(#"Removed Columns6", "margin_pct", each if [revenue] = null or [revenue] = 0 then null
else Number.Round(([revenue] - ([units_sold] * [cost])) / [revenue], 4)),
    #"Changed Type7" = Table.TransformColumnTypes(#"Added Custom9",{{"margin_pct", type number}}),
    #"Added Custom10" = Table.AddColumn(#"Changed Type7", "discount_band", each if [discount_pct] = null then null
else if [discount_pct] = 0 then "no discount"
else if [discount_pct] <= 0.10 then "low (0–10%)"
else if [discount_pct] <= 0.20 then "mid (10–20%)"
else "high (20%+)"),
    #"Changed Type8" = Table.TransformColumnTypes(#"Added Custom10",{{"discount_band", type text}, {"row_quality", type text}, {"units_sold_flag", type text}, {"discount_flag ", type text}, {"profit_flag", type text}})
in
    #"Changed Type8"